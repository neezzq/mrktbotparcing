...
def extract_backdrop_percent(item: dict) -> Any:
    return first_not_none(safe_get(item, "backdropRarityPercent", "backdrop_rarity_percent"), safe_get(item, "backgroundRarityPercent", "background_rarity_percent"), safe_get(item, "backdropRarity", "backdrop_rarity", "backgroundRarity", "background_rarity"))
...
class MrktApi:
    ...
    async def fetch_saling(
        ...
        # MRKT требует эти поля обязательно, даже если они пустые
        payload = {
...
async def calc_avg_sell(api: MrktApi, item: dict) -> Optional[float]:
    try:
        ...
        gifts = result.get("gifts", []) or result.get("items", []) or []
...
async def send_alert(bot: Bot, item: dict, avg_buy: Optional[float], avg_sell: Optional[float]) -> None:
    ...
    await bot.send_message(chat_id=CHAT_ID, text=build_message(item, avg_buy, avg_sell), parse_mode="HTML", disable_web_page_preview=False, reply_markup=markup)
...
async def process_market_page(bot: Bot, api: MrktApi) -> None:
    ...
    for item in reversed(gifts):
        try:
            ...
            if price_ton is None or price_ton > MAX_PRICE_TON:
                continue
